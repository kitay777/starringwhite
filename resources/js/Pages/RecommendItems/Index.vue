<template>
  <AppLayout title="おすすめ商品管理">
    <div class="p-6">
      ...
      <form @submit.prevent="saveItem" class="flex gap-2 mb-6">
        <input v-model="form.title" placeholder="タイトル" class="border p-2 w-1/3" required>
        <input v-model="form.image_url" placeholder="画像URL" class="border p-2 w-1/3">
        <input v-model="form.url" placeholder="リンクURL" class="border p-2 w-1/3">
...
const form = ref({ title: "", image_url: "", url: "" })
...
const resetForm = () => {
  ...
  form.value = { title: "", image_url: "", url: "" }
}
